...
export function AssessmentSidebar({
  ...
  const stars = Array.from({ length: 35 }).map((_, i) => ({
    ...
    floatDur: `${10 + Math.random() * 15}s`,
    size: Math.random() > 0.8 ? "1px" : "0.5px", // Mostly tiny stars
  }));
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-[#120422] via-[#311b92] to-[#7c4dff] text-primary-foreground p-8 flex flex-col overflow-hidden">
      ...
      {mounted && (
        <div className="absolute inset-0 pointer-events-none opacity-60">
          {stars.map((star, idx) => (
            <div
              key={idx}
              className="absolute bg-white rounded-full animate-twinkle animate-float shadow-[0_0_4px_white]"
              style={{
                width: star.size,
                height: star.size,
                top: star.top,
      ...
      <div className="relative z-10 mb-12">
        <h1 className="font-headline text-2xl mb-1 leading-tight">Human-Like Assessment</h1>
        <p className="text-white/60 text-xs font-bold uppercase tracking-widest">Laboratory Session</p>
      </div>

      <div className="relative z-10 flex-1 space-y-2">
        <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-white/30 mb-6 border-b border-white/10 pb-2">Assessment Flow</h2>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isCurrent ? "bg-white/15 shadow-xl backdrop-blur-md border border-white/10 scale-105" : "opacity-50"
              )}
            >
